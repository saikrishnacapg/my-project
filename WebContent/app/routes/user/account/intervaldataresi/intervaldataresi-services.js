angular.module('myaccount.route').service('intervalDataResi', function($filter, ClientStorage) {
...
angular.module('myaccount.route').service('intervalDataResiService', function($filter, HTML5_DATE_FORMAT, intervalDataResi, intervalDataResiHttpServer, IntervalDeviceServer, MyAccountServer, Utils) {
    this.eDevices = [];
    this.gDevices = [];
    ...
    this.init = function(contractAccountNumber, meters) {
        intervalDataResi.clear();

        this.contractAccountNumber = contractAccountNumber;
        this.eDevices = this.filterDevices(meters, intervalDataResi.meterTypes.ELECTRICITY);
        this.gDevices = this.filterDevices(meters, intervalDataResi.meterTypes.GAS);
    ...
    this.search = function(intervalDevices, startDate, endDate) {
        ...
        var promise = intervalDataResiHttpServer.loadElectricityData(params) ;


        var success =  function(result) {
            intervalDataResi.update(result);
        };
        ...
        intervalDataResiHttpServer.intervalDetails(params).then(function(result) {
            intervalDataResi.intervalDetails = result;
        });
    };
    ...
    this.dataSetUrl = function(details) {
        var urlPath = intervalDataResi.isGas() ? (details.downloadType === 'daily' ? "exportDailyGasIntervalData" : "exportHourlyGasIntervalData") :
    ...
    this.exportDataSet = function(details) {
        ...
        intervalDataResiHttpServer.tabularDownload(url);
    ...
    this.emailDataSet = function(details) {
        ...
        intervalDataResiHttpServer.sendDataEmail(url);

        return;
    };
});

angular.module('myaccount.route').service('intervalDataResiHttpServer', function($http, $log, $q, HTML5_DATE_FORMAT, Modals, Utils) {
