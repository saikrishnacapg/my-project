angular.module('myaccount.route').factory('amiIntervalChartServer', function ($log, $q, HTML5_DATE_FORMAT, amiIntervalService, amiIntervalHttpServer) {
	...
	Highcharts.extend(Highcharts.Chart.prototype, {
		emailChart: function (emailOptions, options, chartOptions) {
			...
			var promise = amiIntervalHttpServer.sendChartEmail({
	...
	function consumptionChart(singleDayGraph) {
		...
		consumptionChartDetails.chart.renderTo = 'amiIntervalConsumptionChart';
	...
	function clickNavigator(selectedValue, dayInterval) {
		return function (e) {
			if (parseInt(dayInterval, 10) === 0) {
				amiIntervalService.previousSearch();
			} else {
