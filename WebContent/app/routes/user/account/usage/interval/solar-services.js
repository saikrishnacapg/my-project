angular.module('myaccount.route').service('solar', function ($filter, ClientStorage) {
	...
	this.update = function (result) {
		...
		this.message = this.hasData
			? 'Loading your interval data...'
			: 'Interval data is not available for your current selection.';
	...
	this.tabularData = function () {
		...
		var unbilledStartDate = moment(data.unbilledStartDate).toDate();
		var i = 0;
		angular.forEach(products, function (value, key) {
			var tableData = [];
	...
	this.calcExtraElectricityStats = function (data) {
		...
		for (var i = 0; i < data.numberOfDays; i++) {
			...
			var intervalNo = i * 48;
			var kwHalfHourlyValues = data.kwHalfHourlyValues
				? data.kwHalfHourlyValues.slice(intervalNo, intervalNo + 48)
				: [];
			var kvaHalfHourlyValues = data.kvaHalfHourlyValues
				? data.kvaHalfHourlyValues.slice(intervalNo, intervalNo + 48)
				: [];
			var powerFactorHalfHourlyValues = data.powerFactorHalfHourlyValues
				? data.powerFactorHalfHourlyValues.slice(intervalNo, intervalNo + 48)
				: [];
			var statusCodeHalfHourlyValues = data.statusCodeHalfHourlyValues
				? data.statusCodeHalfHourlyValues.slice(intervalNo, intervalNo + 48)
				: [];
...
angular
	...
	.service('solarService', function (
		...
		this.eDevices = this.filterDevices(meters, solar.meterTypes.ELECTRICITY);
		this.gDevices = this.filterDevices(meters, solar.meterTypes.GAS);
		if (this.eDevices) {
        ...
        this.current.minLimitDate = moment(this.current.moveIN).toDate()> moment(this.current.installDeviceFrom).toDate() ? moment(this.current.moveIN).toDate() : moment(this.current.installDeviceFrom).toDate();*/
			if (this.current.meters.length > 1) {
				...
				this.current.maxLimitDate =
					moment().toDate() > moment(this.current.meters[0].moveOut).toDate()
						? moment(this.current.meters[0].moveOut).toDate()
						: moment().toDate();
				this.current.minLimitDate =
					moment(this.current.moveIN).toDate() > mindaterange ? moment(this.current.moveIN).toDate() : mindaterange;
			} else {
				this.current.installDeviceFrom = moment(this.current.meters[0].installFrom).toDate();
				this.current.installDeviceTo = moment(this.current.meters[0].installTo).toDate();
				this.current.maxLimitDate =
					moment().toDate() > moment(this.current.meters[0].moveOut).toDate()
						...
						: moment().toDate();
				this.current.minLimitDate =
					moment(this.current.moveIN).toDate() > moment(this.current.installDeviceFrom).toDate()
						? moment(this.current.moveIN).toDate()
						: moment(this.current.installDeviceFrom).toDate();
			}
		...
		this.nextInterval = function () {
			...
			var nextEnd = moment(this.current.endDate)
				.add(interval + 1, 'days')
				.format(HTML5_DATE_FORMAT);
			if (moment(this.lastSearched.minLimitDate).isAfter(nextStart)) {
				solar.unAvailableDataFlag=true;
				solar.minDate=moment(this.lastSearched.minLimitDate).format(HTML5_DATE_FORMAT);
			} else {
				solar.unAvailableDataFlag=false;
			}
			this.dateSearch(nextStart, nextEnd);
		};

		this.previousInterval = function () {
			solar.previousFlag=true;
			var interval = this.current.dayInterval || 0;
			var previousStart = moment(this.current.startDate)
		...
		this.previousSearch = function () {
			...
			if (moment(this.current.minLimitDate).isAfter(previousStart)) {
				amiInterval.unAvailableDataFlag=true;
				amiInterval.minDate=this.current.minLimitDate
			} else {
		...
		this.search = function (intervalDevices, startDate, endDate) {
			if (!solar.nextFlag && !solar.previousFlag) {
				solar.unAvailableDataFlag = false;
			}
			solar.nextFlag =false;
			solar.previousFlag=false;
			...
			var promise = solarHttpServer.loadElectricityData(params);
		...
		this.updateSearchHistory = function (intervalDevices, startDate, endDate) {
			...
			this.current.maxLimitDate =
				moment().toDate() > moment(this.current.moveOUT).toDate()
					? moment(this.current.moveOUT).toDate()
					: moment().toDate();
			this.current.installDeviceFrom = moment(this.current.meters[0].installFrom).toDate();
			this.current.installDeviceTo = moment(this.current.meters[0].installTo).toDate();
			this.current.minLimitDate =
		...
		this.dataSetUrl = function (details) {
			var urlPath = solar.isGas() ? details.downloadType === 'daily' ? "exportDailyGasIntervalData" : "exportHourlyGasIntervalData" :
				details.downloadType === 'daily' ? "exportDailyElecIntervalRESIData" : "exportHalfHourlyElecIntervalRESIData";

			let htmllatestBillDate = this.latestBillPeriodEndDate ? encodeURIComponent(Utils.convertToHTML5Date(moment(this.latestBillPeriodEndDate).add(1, 'days'))) : '';
			var url = MyAccountServer + '/intervalData/' + this.contractAccountNumber + '/' + urlPath + '?startDate=' +
				...
				htmllatestBillDate +'&generationTab='+ details.generationTab;
		...
		this.exportDataSet = function (details) {
			...
			solarHttpServer.tabularDownload(url);
		};
		...
		this.emailDataSet = function (details) {
			...
			solarHttpServer.sendDataEmail(url);
		};
	});

angular
	.module('myaccount.route')
	.service('solarHttpServer', function ($http, $log, $q, HTML5_DATE_FORMAT, Modals, Utils) {
		this.loadElectricityData = function (queryParams) {
			...
			var urlParams = {
				...
				unbilledStartDate: queryParams.latestBillPeriodEndDate
					? Utils.convertToHTML5Date(moment(queryParams.latestBillPeriodEndDate).add(1, 'days'))
					: null
			};
			var promise = $http({
				method: 'GET',
